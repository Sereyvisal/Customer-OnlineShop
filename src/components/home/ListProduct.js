...
function getParentCategory(categoryList, id) {
    for (var p of categoryList) {
        if (p._id == id) {
            return p._id;
        }

        if (p.items.length) {
            var found = findChildCategory(p.items, id);
            if (found) {
                return p._id;
            }
        }
...
function findChildCategory(categoryList, id) {
    for (var p of categoryList) {
        if (p._id == id) {
            return true;
        }
    ...
    useEffect(() => {
        if (props.selectedCategory == "0") {
            ...
            if (props.products.length < 4) {
                ...
                setFilterProducts(props.products.slice(0, 4));
            }
            else if (props.products.length >= 8) {
                setFilterProducts(props.products.slice(0, 8));
            }
        }
        else {
            var filter = [];
            for (var p of props.products) {
                var parent_category = getParentCategory(props.categoryList, p.category._id);

                if (parent_category == props.selectedCategory) {
                    filter.push(p);
                }
            }
            ...
            else if (filter.length >= 4 && filter.length < 8) {
                setFilterProducts(filter.slice(0, 4));
            }
            else if (filter.length >= 8) {
                setFilterProducts(filter.slice(0, 8));
            }
        ...
        <>
            {
                filterProducts && filterProducts.map((product, index) => (
                    ...
                    >
                        <div className="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden h-96 lg:h-80 lg:aspect-none">
                            <img
